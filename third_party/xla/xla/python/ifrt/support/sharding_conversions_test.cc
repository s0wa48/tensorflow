...
TEST_P(ShardingConversionsTest, OpShardingUnreduced) {
  ...
  ShardingParam expected{/*dim_shards=*/{1, 1},
                         {/*permutation=*/{0}, /*axis_sizes=*/{6}},
                         /*unreduced_axes=*/std::vector<int>{0}};
  TF_EXPECT_OK(expected.verify());
  EXPECT_EQ(actual, expected);
}
